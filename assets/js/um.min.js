...
this.data("tipsy");if(f)f[a]();return this}a=b.extend({},b.fn.tipsy.defaults,a);a.live||this.each(function(){c(this)});if(a.trigger!="manual"){f=a.live?"live":"bind";var g=a.trigger=="hover"?"mouseleave":"blur";this[f](a.trigger=="hover"?"mouseenter":"focus",d)[f](g,h)}return this};b.fn.tipsy.defaults={className:null,delayIn:0,delayOut:0,fade:false,fallback:"",gravity:"n",html:false,live:false,offset:0,opacity:0.8,title:"title",trigger:"hover"};b.fn.tipsy.revalidate=function(){b(".tipsy").each(function(){var a=
...
jQuery(window).load(function(){um_responsive();um_modal_responsive()});jQuery(window).resize(function(){jQuery(".um-modal .um-single-image-preview.crop:visible img").cropper("destroy");um_responsive();um_modal_responsive()});jQuery(document).ready(function(){jQuery(document).on("click",'.um-modal-overlay, a[data-action="um_remove_modal"]',function(){um_remove_modal()});jQuery(document).on("click",'a[data-modal^="um_"], span[data-modal^="um_"], .um-modal a',function(a){a.preventDefault();return false});jQuery(document).on("click",".um-modal .um-single-file-preview a.cancel",function(a){a.preventDefault();a=jQuery(this).parents(".um-modal-body");a.find(".um-single-file-preview").hide();a.find(".ajax-upload-dragdrop").show();
a.find(".um-modal-btn.um-finish-upload").addClass("disabled");um_modal_responsive();return false});jQuery(document).on("click",".um-modal .um-single-image-preview a.cancel",function(a){a.preventDefault();a=jQuery(this).parents(".um-modal-body");a.find(".um-modal .um-single-image-preview img").cropper("destroy");a.find(".um-single-image-preview img").attr("src","");a.find(".um-single-image-preview").hide();a.find(".ajax-upload-dragdrop").show();a.find(".um-modal-btn.um-finish-upload").addClass("disabled");
um_modal_responsive();return false});jQuery(document).on("click",".um-finish-upload.file",function(){var a=jQuery(this).attr("data-key"),b=jQuery(this).parents(".um-modal-body").find(".um-single-file-preview").html();um_remove_modal();jQuery(".um-single-file-preview[data-key="+a+"]").fadeIn().html(b);jQuery(".um-single-file-preview[data-key="+a+"]").parents(".um-field").find(".um-btn-auto-width").html(jQuery(this).attr("data-change"));jQuery(".um-single-file-preview[data-key="+a+"]").parents(".um-field").find("input[type=hidden]").val(jQuery(".um-single-file-preview[data-key="+
a+"]").parents(".um-field").find(".um-single-fileinfo a").attr("href"))});jQuery(document).on("click",".um-finish-upload.image",function(){var a=jQuery(this),b=jQuery(this).attr("data-key"),c=jQuery(this).parents(".um-modal-body").find(".um-single-image-preview"),f=c.find("img").attr("src");c=c.attr("data-coord");var g=jQuery(this).parents("#um_upload_single").attr("data-user_id")?jQuery(this).parents("#um_upload_single").attr("data-user_id"):0;if(c){jQuery(this).html(jQuery(this).attr("data-processing")).addClass("disabled");
jQuery.ajax({url:ultimatemember_ajax_url,type:"POST",data:{action:"ultimatemember_resize_image",src:f,coord:c,user_id:g,key:b},success:function(e){d=new Date;b=="profile_photo"&&jQuery(".um-profile-photo-img img").attr("src",e+"?"+d.getTime());if(b=="cover_photo"){jQuery(".um-cover-e").empty().html('<img src="'+e+"?"+d.getTime()+'" alt="" />');jQuery(".um").hasClass("um-editing")&&jQuery(".um-cover-overlay").show()}jQuery(".um-single-image-preview[data-key="+b+"]").fadeIn().find("img").attr("src",
e+"?"+d.getTime());um_remove_modal();jQuery(".um-single-image-preview[data-key="+b+"]").parents(".um-field").find(".um-btn-auto-width").html(a.attr("data-change"));jQuery(".um-single-image-preview[data-key="+b+"]").parents(".um-field").find("input[type=hidden]").val(e)}})}else{d=new Date;jQuery(".um-single-image-preview[data-key="+b+"]").fadeIn().find("img").attr("src",f+"?"+d.getTime());um_remove_modal();jQuery(".um-single-image-preview[data-key="+b+"]").parents(".um-field").find(".um-btn-auto-width").html(a.attr("data-change"));
jQuery(".um-single-image-preview[data-key="+b+"]").parents(".um-field").find("input[type=hidden]").val(f)}});jQuery(document).on("click",'a[data-modal^="um_"], span[data-modal^="um_"]',function(){var a=jQuery(this).attr("data-modal"),b=jQuery(this).data("modal-size")?jQuery(this).data("modal-size"):"normal";if(jQuery(this).data("modal-copy")){jQuery("#"+a).html(jQuery(this).parents(".um-field").find(".um-modal-hidden-content").html());jQuery(this).parents(".um-profile-photo").attr("data-user_id")&&
jQuery("#"+a).attr("data-user_id",jQuery(this).parents(".um-profile-photo").attr("data-user_id"));jQuery(this).parents(".um-cover").attr("data-ratio")&&jQuery("#"+a).attr("data-ratio",jQuery(this).parents(".um-cover").attr("data-ratio"));jQuery(this).parents(".um-cover").attr("data-user_id")&&jQuery("#"+a).attr("data-user_id",jQuery(this).parents(".um-cover").attr("data-user_id"))}um_new_modal(a,b)});jQuery(document).on("input",".um-field input[type=text]",function(){live_field=jQuery(this).parents(".um-field").data("key");live_value=jQuery(this).val();um_conditional()});jQuery(".um-field input[type=text]").trigger("input");
jQuery(document).on("change",".um-field select, .um-field input[type=radio], .um-field input[type=checkbox]",function(){live_field=jQuery(this).parents(".um-field").data("key");live_value=jQuery(this).val();if(jQuery(this).is(":checkbox"))live_value=jQuery(this).parents(".um-field").find("input:checked").val();um_conditional()});jQuery(document).on("click",".um-dropdown a",function(){return false});jQuery(document).on("click",".um-dropdown a.real_url",function(){window.location=jQuery(this).attr("href")});
jQuery(document).on("click",".um-trigger-menu-on-click",function(){jQuery(".um-dropdown").hide();menu=jQuery(this).find(".um-dropdown");menu.show();return false});jQuery(document).on("click",".um-dropdown-hide",function(){UM_hide_menus()});jQuery(document).on("click","a.um-manual-trigger",function(){var a=jQuery(this).attr("data-child"),b=jQuery(this).attr("data-parent");jQuery(this).parents(b).find(a).trigger("click")});jQuery(".um-tip-n").tipsy({gravity:"n",opacity:1,live:true,offset:3});jQuery(".um-tip-w").tipsy({gravity:"w",
...
jQuery(".um-field input[type=checkbox]").change(function(){jQuery(this).parents(".um-field");var a=jQuery(this).parents("label");if(a.hasClass("active")){a.removeClass("active");a.find("i").removeClass().addClass("um-icon-android-checkbox-outline-blank")}else{a.addClass("active");a.find("i").removeClass().addClass("um-icon-android-checkbox-outline")}});jQuery('.um-datepicker').each(function(){elem = jQuery(this);if(elem.attr('data-disabled_weekdays')!= ''){var disable = JSON.parse(elem.attr('data-disabled_weekdays'))}else{var disable = false}var years_n = elem.attr('data-years');var min = new Date(elem.attr('data-date_min'));var max = new Date(elem.attr('data-date_max'));elem.pickadate({selectYears:years_n,min:min,max:max,disable:disable,format:elem.attr('data-format'),formatSubmit:'yyyy/mm/dd',hiddenName:true})});jQuery(".um-timepicker").each(function(){elem=jQuery(this);elem.pickatime({format:elem.attr("data-format"),
interval:parseInt(elem.attr("data-intervals")),formatSubmit:"HH:i",hiddenName:true})});jQuery(".um-rating").raty({half:false,starType:"i",number:function(){return jQuery(this).attr("data-number")},score:function(){return jQuery(this).attr("data-score")},scoreName:function(){return jQuery(this).attr("data-key")},hints:false,click:function(a){live_field=this.id;live_value=a;um_conditional()}});jQuery(".um-rating-readonly").raty({half:false,starType:"i",number:function(){return jQuery(this).attr("data-number")},
score:function(){return jQuery(this).attr("data-score")},scoreName:function(){return jQuery(this).attr("data-key")},hints:false,readOnly:true});jQuery(document).on("click",".um .um-single-image-preview a.cancel",function(a){a.preventDefault();a=jQuery(this).parents(".um-field");a.find(".um-single-image-preview img").attr("src","");a.find(".um-single-image-preview").hide();a.find(".um-btn-auto-width").html("Upload");a.find("input[type=hidden]").val("");return false});jQuery(document).on("click",".um .um-single-file-preview a.cancel",
function(a){a.preventDefault();a=jQuery(this).parents(".um-field");a.find(".um-single-file-preview").hide();a.find(".um-btn-auto-width").html("Upload");a.find("input[type=hidden]").val("");return false});jQuery(".um-s1").select2({allowClear:true,minimumResultsForSearch:10});jQuery(".um-s2").select2({allowClear:false,minimumResultsForSearch:10});jQuery(".um-s1,.um-s2").css({display:"block"});jQuery(document).on("click",".um-field-group-head:not(.disabled)",function(){var a=jQuery(this).parents(".um-field-group"),
b=a.data("max_entries");a.find(".um-field-group-body").is(":hidden")?a.find(".um-field-group-body").show():a.find(".um-field-group-body:first").clone().appendTo(a);increase_id=0;a.find(".um-field-group-body").each(function(){increase_id++;jQuery(this).find("input").each(function(){var c=jQuery(this);c.attr("id",c.data("key")+"-"+increase_id);c.attr("name",c.data("key")+"-"+increase_id);c.parent().parent().find("label").attr("for",c.data("key")+"-"+increase_id)})});b>0&&a.find(".um-field-group-body").length==
b&&jQuery(this).addClass("disabled")});jQuery(document).on("click",".um-field-group-cancel",function(a){a.preventDefault();a=jQuery(this).parents(".um-field-group");var b=a.data("max_entries");a.find(".um-field-group-body").length>1?jQuery(this).parents(".um-field-group-body").remove():jQuery(this).parents(".um-field-group-body").hide();b>0&&a.find(".um-field-group-body").length<b&&a.find(".um-field-group-head").removeClass("disabled");return false});jQuery(document).ready(function(){jQuery('.um-members').each(function(){UM_Member_Grid(jQuery(this))});jQuery('.um-member-connect').each(function(){if(jQuery(this).find('a').length == 0){jQuery(this).remove()}});jQuery('.um-member-meta-main').each(function(){if(jQuery(this).find('.um-member-metaline').length == 0 && jQuery(this).find('.um-member-connect').find('a').length == 0){jQuery(this).remove()}});jQuery(document).on('click','.um-member-more a',function(e){e.preventDefault();var block = jQuery(this).parents('.um-member');var container = jQuery(this).parents('.um-members');block.find('.um-member-more').hide();block.find('.um-member-meta').slideDown(function(){UM_Member_Grid(container)});block.find('.um-member-less').fadeIn();setTimeout(function(){UM_Member_Grid(container)},100);return false});jQuery(document).on('click','.um-member-less a',function(e){e.preventDefault();var block = jQuery(this).parents('.um-member');var container = jQuery(this).parents('.um-members');block.find('.um-member-less').hide();block.find('.um-member-meta').slideUp(function(){block.find('.um-member-more').fadeIn();UM_Member_Grid(container)});return false});jQuery(document).on('click','.um-do-search',function(e){e.preventDefault();jQuery(this).parents('form').submit();return false})});jQuery(document).on("click",
".um-profile-save",function(a){a.preventDefault();jQuery(this).parents(".um").find("form").submit();return false});jQuery(document).on("click",".um-profile-edit-a",function(){jQuery(this).addClass("active")});jQuery(document).on("click",".um-cover a, .um-photo a",function(a){a.preventDefault();return false});jQuery(document).on("click",".um-photo-modal",function(){var a=jQuery(this).attr("data-src");um_new_modal("um_view_photo","fit",true,a)});jQuery(document).on("click",".um-reset-profile-photo",
function(){jQuery(".um-profile-photo-img img").attr("src",jQuery(this).attr("data-default_src"));user_id=jQuery(this).attr("data-user_id");metakey="profile_photo";jQuery.ajax({url:ultimatemember_ajax_url,type:"post",data:{action:"ultimatemember_delete_profile_photo",metakey:metakey,user_id:user_id}})});jQuery(document).on("click",".um-reset-cover-photo",function(){jQuery(".um-cover-overlay").hide();jQuery(".um-cover-e").html('<a href="#" class="um-cover-add um-manual-trigger" data-parent=".um-cover" data-child=".um-btn-auto-width"><span class="um-cover-add-i"><i class="um-icon-plus um-tip-n" title="Upload a cover photo"></i></span></a>');
jQuery(".um-dropdown").hide();um_responsive();user_id=jQuery(this).attr("data-user_id");metakey="cover_photo";jQuery.ajax({url:ultimatemember_ajax_url,type:"post",data:{action:"ultimatemember_delete_cover_photo",metakey:metakey,user_id:user_id}})});var g=jQuery(".um-account-main").attr("data-current_tab");g&&jQuery(".um-account-tab[data-tab="+g+"]").show();jQuery(document).on("click",".um-account-side li a",function(a){a.preventDefault();a=jQuery(this);a.parents("ul").find("li a").removeClass("current");
